...
def test_mover_64_net(env):  # whole model
    from PIL import Image
    from src.utils.MCTS import MCT
    # import imageio
    ...
    # pre_action = np.zeros([N_worker, action_size])
    ...
    frame_cnt = 0
    while (not (finished_test == 1)) and s < 40:
        s += 1
        max_depth = 0
...
if __name__ == '__main__':
    ...
    for i in range(N_worker):
        print(i)
        t_start = time.time()
        envs = DataIO('/home/baifan/RL/SM-New/SceneMover/data/train_IL.pkl', N_worker)
        _ = envs.reset()
        env = envs.envs[i]
        finished_test,total_reward,s,length = test_mover_64_net(env)
        t_end = time.time()
