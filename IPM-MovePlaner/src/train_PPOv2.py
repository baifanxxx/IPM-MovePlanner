...
imitate_models_path = 'imitate_models/9100-model.pth'
...
def main():
    ...
    env = DataIO('/home/baifan/RL/SM-New/SceneMover/data/train_IL.pkl', N_worker)
    ...
    while step <= train_max_step:
        ...
        for _ in range(T_horizon):
            ...
            # logits_cpu = prob.detach().cpu().numpy()
        ...
        if step % save_freq == 0 and step != 0:
            # the_dir, _ = os.path.split(weight_path)
            ...
            for param in all_tuple:
                create_state(*param)
            torch.save(states, weight_path + '/' + str(step) + '-model.pth')
