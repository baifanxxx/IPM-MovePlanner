...
from src.utils.env import ENV as classic
from src.utils.env import ENV_ablation_wo_base as base
from src.utils.env import ENV_ablation_wo_multi as multi
from src.utils.env import ENV_ablation_wo_repetition as repetition
    ...
    def make_action_logits(self, logits):
        ...
        for i,lo in enumerate(logits):
            while True:
                ...
                #     noise = np.random.rand(lo.shape[0])
                #     lo = lo - np.log(-np.log(noise))
                # a = lo.argmax()
                # item = int(a / self.action_type)
                # d = int(a % self.action_type)
                # reward, flag = self.envs[i].move(item, d)
